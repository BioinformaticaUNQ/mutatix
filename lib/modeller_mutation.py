...
#  mutate_model.py
#
#     Usage:   python mutate_model.py modelname respos resname chain outputfile > logfile
#
...
def mutate_by_residue_pos(modelname, respos_, restyp, chain, outputfile = None):
  ...
  mdl1 = model(env, file=modelfile)
  ali = alignment(env)
  ali.append_model(mdl1, atom_files=modelfile, align_codes=modelfile)
  ...
  ali.append_model(mdl1, align_codes=modelfile)
  ...
  mdl1.write(file=modelfile+restyp+respos+'.tmp')
  mdl1.read(file=modelfile+restyp+respos+'.tmp')
  ...
  #give a proper name
  mdl1.write(file=(outputfile if outputfile else modelfile+restyp+respos+'.pdb'))

  #delete the temporary file
  os.remove(modelfile+restyp+respos+'.tmp')


if __name__ == '__main__':
  outputfile = None
  try :
    modelname, respos, restyp, chain, outputfile = sys.argv[1:]
  except :
    modelname, respos, restyp, chain = sys.argv[1:]

  mutate_by_residue_pos(modelname, respos, restyp, chain, outputfile)